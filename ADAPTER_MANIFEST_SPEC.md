...
- **Madde 6** (LLM dış tercüman): LLM, `intent_relay` / translator capability profiline sahip bir dış adapter'dır; constitutional adapter type **değildir** ve execution adapter'a doğrudan yol yoktur.
...
> *Canonical verification row lives in [`MEMORY_WRITE_GATE.md`](./MEMORY_WRITE_GATE.md) §8; this section mirrors it for adapter context. "Matrix satırı olmayan subject_class verified olamaz" invariant'ı G §8'de zorunlu.*
...
| deactivated → revoked | security incident, audit violation, manifest compromise | insan + DEONTIC review |
